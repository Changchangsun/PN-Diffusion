...
class AudioDiffusion(Dataset):
    def __init__(self, model_id, channels=3,stage="train"):
        super().__init__()
        ...
        if os.path.exists(model_id):
            if stage =="train":
                ...
                self.hf_dataset = load_from_disk(model_id)["test"]
            elif stage=="val":
                self.hf_dataset = load_from_disk(model_id)["val"]
...
class AudioDiffusionDataModule(pl.LightningDataModule):
    def __init__(self, model_id, batch_size, channels,stage):
        super().__init__()
        self.batch_size = batch_size
        self.dataset = AudioDiffusion(model_id=model_id, channels=channels,stage=stage)
        self.num_workers = 1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ldm_checkpoint_dir", type=str, default="vae_models/aist_5s_new_resume2/ldm-autoencoder-kl")
    parser.add_argument("--hf_checkpoint_dir", type=str, default="vae_models/aist_5s_new_resume2/autoencoder-kl")
    ...
    parser.add_argument("--save_images_batches", type=int, default=500)
    ...
    model = torch.nn.DataParallel(model, device_ids=[1,2])  # 指定要使用的GPU设备
...
"""
Obtain VAE model based on pre-obtained Mel spectrogram images.
Run command:
# python scripts/train_vae.py \
# --dataset_name /images_root_dir \
# --batch_size 12 \
