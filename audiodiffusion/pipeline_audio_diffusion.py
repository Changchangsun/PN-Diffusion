...
class AudioDiffusionPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        encoding_motion: torch.Tensor = None,################ 1
        encoding_neg: torch.Tensor = None,
        encoding_motion_neg: torch.Tensor = None,  ################ 1
        return_dict=True,
    ) -> Union[
        ...
        for step, t in enumerate(self.progress_bar(self.scheduler.timesteps[start_step:])):
            if isinstance(self.unet, UNet2DConditionModel):
                ...
                # model_output = self.unet(images, t, encoding,encoding_motion)["sample"]#################  2
                # model_output = self.unet(images, t, encoding,encoding_motion,encoding_neg,encoding_motion_neg)["sample"]#################  2
                model_output = self.unet(images, t, encoding,encoding_motion,encoding_neg,encoding_motion_neg)#################  2
                #RuntimeError: mat1 and mat2 shapes cannot be multiplied (16x3072 and 1024x1280)
            else:
            ...
            if isinstance(self.scheduler, DDIMScheduler):
                images = self.scheduler.step(
                    ...
                    model_output=model_output[0]["sample"],
                    timestep=t,
                    sample=images,
                    eta=eta,
