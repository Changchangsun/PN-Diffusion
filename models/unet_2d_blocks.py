...
# num_layers********************* 2
# 0 iiiiiiiiiiiiiiiiiii False
# 1 iiiiiiiiiiiiiiiiiii False
# num_layers********************* 2
# 0 iiiiiiiiiiiiiiiiiii False
# 1 iiiiiiiiiiiiiiiiiii False
# num_layers********************* 2
...
# 1 iiiiiiiiiiiiiiiiiii False
        ...
        for i in range(num_layers):
            # print(i,"iiiiiiiiiiiiiiiiiii",dual_cross_attention)
            in_channels = in_channels if i == 0 else out_channels
            resnets.append(######
                ResnetBlock2D(
                    in_channels=in_channels,
    ...
    def forward(
        ...
        for resnet, attn in zip(self.resnets, self.attentions):
            if self.training and self.gradient_checkpointing:
                ...
                # print("!!!!!!!!!!!!!!hidden_states",hidden_states.shape,temb.shape)
                #!!!!!!!!!!!!!!hidden_states torch.Size([8, 128, 32, 32]) torch.Size([8, 512])
                hidden_states = resnet(hidden_states, temb)
                # !!!!!!!!!!!!!!hidden_states1 torch.Size([8, 128, 32, 32])
# !!!!!!!!!!!!!!!!!!2encoder_hidden_states torch.Size([8, 1, 3072])
                # print("!!!!!!!!!!!!!!hidden_states1",hidden_states.shape)
                # print("!!!!!!!!!!!!!!!!!!2encoder_hidden_states",encoder_hidden_states.shape)
                hidden_states = attn(
                    hidden_states,
                    encoder_hidden_states=encoder_hidden_states,
                    cross_attention_kwargs=cross_attention_kwargs,
